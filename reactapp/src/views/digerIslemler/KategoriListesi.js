    ...
    const { data, isError, isFetching, isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            ...
            let config = {
                ...
                url: 'http://localhost:5273/api/Category/GetGrid',
                data: data
            };
    ...
    const deletePromise = (id) => {
        return new Promise(async (resolve, reject) => {
            ...
            let config = {
                method: 'get',
                maxBodyLength: Infinity,
                url: 'http://localhost:5273/api/Category/Delete',
                headers: {
                ...
                renderRowActions={({ row }) => (
                    <Box sx={{ display: 'flex', flexWrap: 'nowrap', gap: '8px' }}>
                        ...
                        <IconButton
                            color="secondary"
                            onClick={() => {
                                navigate(`/digerIslemler/kategori-duzenle/${row.original.id}`);
                            }}
                        >
                            <EditIcon />
                        </IconButton>
